...
class AllReduceCoalesced : public DistPrimitive {
 public:
  enum ReduceType { Sum };

  AllReduceCoalesced(Stream stream, Group group, ReduceType reduce_type)
      : DistPrimitive(stream, group), reduce_type_(reduce_type) {}

  void eval_cpu(const std::vector<array>& inputs, std::vector<array>& outputs)
      override {
    throw std::runtime_error(
        "[AllReduceCoalesced] CPU evaluation is not supported.");
  };
  ...
  std::pair<std::vector<array>, std::vector<int>> vmap(
      ...
      const std::vector<int>& axes) override {
    throw std::runtime_error(
        "[AllReduceCoalesced] Vectorized map (vmap) is not supported for this primitive.");
  }
  ...
  std::vector<array> jvp(
      ...
      const std::vector<array>& tangents,
      const std::vector<int>& argnums) override {
    throw std::runtime_error("[AllReduceCoalesced] JVP is not supported.");
  }
